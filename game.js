...
let timerInterval;
let topPlayers = [];
let correctAnswers = 0; // מונה לתשובות נכונות
...
function startLevel() {
    score = 0;
    correctAnswers = 0; // איפוס מונה התשובות הנכונות
    $('#points').text(score);
    startTimer();
    generateQuestion();
}
...
function checkAnswer(selectedAnswer, correctAnswer) {
    if (selectedAnswer === correctAnswer) {
        score += 50;
        correctAnswers++; // הגדלת מונה התשובות הנכונות
    } else {
...
function endGame() {
    ...
    $('#end-message').text(`כל הכבוד ${playerName}, בחרת רמת קושי ${currentLevel} והצלחת להשיג ${score} נקודות עם ${correctAnswers} תשובות נכונות בדקה אחת!`);
...
function updateTopPlayers() {
    topPlayers.push({ name: playerName, level: currentLevel, score: score, correctAnswers: correctAnswers });
